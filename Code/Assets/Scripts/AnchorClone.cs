...
public class AnchorClone : MonoBehaviour {
    ...
    void checkDeletion()
    {
     ...
     //   Debug.Log(transform.position + " " + deletionPlane.transform.position);

       // Debug.Log(Vector3.Distance(deletionPlane.transform.position, transform.position));

        var vectorToTarget = transform.position - deletionPlane.transform.position;

        //Debug.Log(vectorToTarget.y);
